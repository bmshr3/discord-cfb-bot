...
# Helper: Fetch ESPN scoreboard (Thu-Sun filter)
async def fetch_espn_scoreboard():
    url = "https://site.api.espn.com/apis/site/v2/sports/football/college-football/scoreboard"
    # Filter for Thu-Sun: Start Thursday, end next Monday
    thursday = datetime.utcnow() - timedelta(days=datetime.utcnow().weekday() % 7)  # Thursday
    sunday_end = thursday + timedelta(days=4)  # Monday morning
    params = {
        "dates": f"{thursday.strftime('%Y%m%d')}-{sunday_end.strftime('%Y%m%d')}"
    }
...
async def cfbscore(interaction: discord.Interaction, team: str):
    try:
        ...
        await interaction.response.send_message("No current weekend game found for that team.")
